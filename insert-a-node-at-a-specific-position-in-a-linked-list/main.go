...
func insertNodeAtPosition(llist *SinglyLinkedListNode, data int32, position int32) *SinglyLinkedListNode {
	insert(llist, data, position-1)
	return llist
}

func insert(llist *SinglyLinkedListNode, data, position int32) {
	if position == 0 {
		llist.next = &SinglyLinkedListNode{data, llist.next}
		return
	}

	insert(llist.next, data, position-1)
}
